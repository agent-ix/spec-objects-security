...
- Every one of the twenty-three object types SHALL admit 0 defaulted fields: a `FieldDecl` carrying `default` is refused, so a declaration cannot carry the material or the grade it is about.
- The rule SHALL cover the types that grant, not only the types that hold material. Restricting it to `secret`, `encryption_key`, `jwt_claim` and `csrf_token` leaves it off exactly the rows the merge gate names: a `control` whose `effectiveness` defaults to `effective`, a `trust_boundary` whose `trust_level` defaults to `trusted`, and a `role`, `permission` or `scope` whose grant row carries a default.
- Each type that admits `operations` SHALL apply the same refusal to `operations[].params[]`, whose items are `FieldDecl`s too. Without it the guard reaches the declaration's own fields and misses the parameter list, which is exactly where an authentication exchange names its code, verifier and token.
...
- `audit_event` is covered by the same rule for its own reason: an audit declaration that carries a literal payload value is carrying the record rather than declaring it.
...
| FR-006-AC-4 | For each of the twenty-three types, a record carrying a field with a `default` — or, where the type admits `operations`, an operation parameter with a `default` — fails its schema, while the same record without it validates. The defaulted `FieldDecl` used to test this is itself a valid semantic-core `FieldDecl`, so the module's own rule is what refuses it rather than the grammar. | Test |
