...
- Because those keys are optional, the obligation each of them carries SHALL also be expressed as an item rule over `fields`, which the extractor does populate: a threat states its STRIDE class as a `stride_category` row and a control its `effectiveness` row, one row per optional graded key its table row names, so the obligation is enforced today rather than deferred.
